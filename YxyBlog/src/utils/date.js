 ...
 * @return {String}         非标准的对用户友好的日期字符串
 */
export const getDateTime = function (dateStr) {
  try {
    let pureDateStr = dateStr.slice(0, 19)
    let date = pureDateStr.split('T').join(' ')
    return date
  } catch (err) {
    return ''
  }
...
export const getMonthInfo = function ({year, month}) {
  let jsMonth = month - 1
...
export const getMonthFormatInfo = function ({year, month}) {
  let jsMonth = month - 1
  let monthDayNum = new Date(year, jsMonth + 1, 0).getDate()
  let firstDay = new Date(year, jsMonth, 1).getDay()
  let dateFirstDay = firstDay == 0 ? 7 : firstDay
  return {
    monthDayNum,
    dateFirstDay
  }
...
export const getArrayViaCountFunc = function (count, func) {
  let ary = []
